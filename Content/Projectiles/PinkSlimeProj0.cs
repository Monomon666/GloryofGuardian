...
{
    public class PinkSlimeProj0 : GOGProj
    {
        //不使用贴图,重写绘制
        ...
        public override void OnKill(int timeLeft) {
            ...
            SoundEngine.PlaySound(in SoundID.Item154, Projectile.Center);
            ...
            if (Projectile.ai[0] == 0) {
                for (int i = -2; i <= 2; i++) {
                    ...
                    Projectile proj1 = Projectile.NewProjectileDirect(new EntitySource_Parent(Projectile), Projectile.Center, vel.RotatedBy(MathHelper.PiOver4 / 6 * i), ModContent.ProjectileType<PinkSlimeProj>(), lastdamage, 1, Owner.whoAmI);
            ...
            if (Projectile.ai[0] == 1) {
                for (int i = -4; i <= 4; i++) {
                    Vector2 vel = new Vector2(0, -8);
                    Projectile proj1 = Projectile.NewProjectileDirect(new EntitySource_Parent(Projectile), Projectile.Center, vel.RotatedBy(MathHelper.PiOver4 / 8 * i), ModContent.ProjectileType<PinkSlimeProj>(), lastdamage, 1, Owner.whoAmI);
                    if (Projectile.ModProjectile is GOGProj proj0 && proj0.OrichalcumMarkProj) {
                        if (proj1.ModProjectile is GOGProj proj2) {
                            proj2.OrichalcumMarkProj = true;
                            proj2.OrichalcumMarkProjcount = 300;
        ...
        public override bool PreDraw(ref Color lightColor) {
            ...
            Texture2D texture0 = ModContent.Request<Texture2D>(GOGConstant.Projectiles + "PinkSlimeDT").Value;
            Texture2D texture1 = ModContent.Request<Texture2D>(GOGConstant.Projectiles + "PinkSlimeDT2").Value;
