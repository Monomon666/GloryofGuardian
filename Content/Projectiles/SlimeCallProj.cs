...
{
    public class SlimeCallProj : GOGProj
    {
                ...
                if (reboundcount <= 0) {
                    if (Projectile.velocity.Y <= 0) {
                        ...
                        if (Main.rand.Next(100) >= Owner.GetCritChance<GenericDamageClass>() + (int)Projectile.ai[1]) {
                            Projectile proj1 = Projectile.NewProjectileDirect(new EntitySource_Parent(Projectile), Projectile.Center, new Vector2(0, 64), ModContent.ProjectileType<SlimeProj0>(), Projectile.damage, 0, Owner.whoAmI, 0);
                            if (Projectile.ModProjectile is GOGProj proj0 && proj0.OrichalcumMarkProj) {
                                if (proj1.ModProjectile is GOGProj proj2) {
                                    proj2.OrichalcumMarkProj = true;
                                    proj2.OrichalcumMarkProjcount = 300;
                        ...
                        if (Main.rand.Next(100) < Owner.GetCritChance<GenericDamageClass>() + (int)Projectile.ai[1]) {
                            Projectile proj1 = Projectile.NewProjectileDirect (new EntitySource_Parent(Projectile), Projectile.Center, new Vector2(0, 64), ModContent.ProjectileType<SlimeProj0>(), Projectile.damage, 0, Owner.whoAmI, 1);
                            if (Projectile.ModProjectile is GOGProj proj0 && proj0.OrichalcumMarkProj) {
                                if (proj1.ModProjectile is GOGProj proj2) {
                                    proj2.OrichalcumMarkProj = true;
                                    proj2.OrichalcumMarkProjcount = 300;
